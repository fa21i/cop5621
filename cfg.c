...
void dup_optim(struct cfg* t, int sz, struct node_fun_str* fun_r){
  //check parent blocks instr, delete one block and recconect br
  while (fun_r != NULL) {
    fprintf (stderr, "\nfunction %s\n", fun_r->name);
    for (int i = 0; i <= sz; i++) {
      struct cfg *r = t;
...
void unr_optim(struct cfg* t, int sz, struct node_fun_str* fun_r){
  //check for if branches, if always true/false, delete until reconnect
  while (fun_r != NULL) {
    fprintf (stderr, "\nfunction %s\n", fun_r->name);
    for (int i = 0; i <= sz; i++) {
      struct cfg *r = t;
...
void optim(struct cfg* t, int sz, struct node_fun_str* fun_r)
{
  basic_optim(t, sz, fun_r);
  dup_optim(t, sz, fun_r);
  unr_optim(t, sz, fun_r);
}